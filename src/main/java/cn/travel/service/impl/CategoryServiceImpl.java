...
import cn.travel.service.CategoryService;
...
//
//            //4.如果不为空,将set的数据存入list(要保证页面的显示顺序)，注意现在只是传了cname
...
//    }
    ...
    public List<Category> findAll() {
        ...
        if (categorys == null || categorys.size() == 0) {
            ...
            System.out.println("从redis中查询.....");
            //4.如果不为空,将set的数据存入list(要保证页面的显示顺序),即保存了cid也保存了cname
            cs = new ArrayList<Category>();
